...
extern USHORT CRC16 (BYTE * puchMsg, USHORT usDataLen, USHORT prevCRC);


static void save_crc(FATFS *fs)
{
    int i = 0;
    DWORD n = 0;
    DWORD f_sect = fs->fatbase;
    USHORT crc = 0xFFFF;
    for(f_sect = fs->fatbase; f_sect < (fs->fatbase + fs->fsize); f_sect++) //  Читаем все секторы с FAT
    {
        disk_read(fs->drv, fs->win, f_sect, 1);

        crc = CRC16(((BYTE *)fs->win + 2), SS(fs) - 2, crc);  // подсчет crc для прочитанного сектора
    }

    disk_read(fs->drv, fs->win, fs->fatbase, 1);  //  Читаем первый сектор fAT где хранится CRC

    n = crc;
    n |= 0xFFFF0000;
    ST_DWORD(fs->win + 0, n);	//  добовляем в самое начало crc
     

    f_sect = fs->fatbase;                          //  устанавливаем сектор на гачало FAT 
    for (i = 0; i < fs->n_fats; i++)
    {
	disk_write(fs->drv, fs->win, f_sect, 1);
        f_sect += fs->fsize;                        // перемещаемся на копию
    } 
}
...
//	USHORT crc;
	...
	if (wsect != sector) {	/* Changed current window */
		...
		/*	if(wsect == 0x42)
			{
				disk_read(fs->drv, fs->win, wsect, 1);
				disk_read(fs->drv, fs->win, wsect, 1);
				disk_read(fs->drv, fs->win, wsect, 1);
				disk_read(fs->drv, fs->win, wsect, 1);
				crc = CRC16(((BYTE *)fs->win), fs->ssize, 0xFFFF);
				if(crc != g_crc)
					g_crc = crc;   
			}  	*/
			fs->wflag = 0;
			if (fs->fatbase <= wsect && wsect < (fs->fatbase + fs->fsize)) {	/* In FAT area */
				BYTE nf;
...
)
{
	...
	if (res == FR_OK) {
		...
		//	g_crc = CRC16(((BYTE *)dj->fs->win), dj->fs->ssize, 0xFFFF);
		}
	}
#endif
...
)
{
	...
	*rfs = fs = FatFs[vol];				/* Return pointer to the corresponding file system object */
	...
	if (fmt == 1) {						/* Not an FAT-VBR, the disk may be partitioned */
		...
		tbl = &fs->win[MBR_Table + (vol) * 16];	/* Partition table */
...
//	}
//
//        DWORD bsect = 63, fasize = 0, f_sect = 0, crc = 0;
//        WORD nrsv = 0;
//        USHORT calc_crc_1 = 0xFFFF, read_crc_1 = 0, calc_crc_2 = 0xFFFF, read_crc_2 = 0;
//        BYTE b;
//
//        disk_read(fs->drv, fs->win, bsect, 1);
//
//        fasize = LD_WORD(fs->win+BPB_FATSz16);				/* Number of sectors per FAT */
//	if (!fasize) fasize = LD_DWORD(fs->win+BPB_FATSz32);
//	fs->fsize = fasize;
...
//	if (!nrsv) return FR_NO_FILESYSTEM;
	   ...
	   return FR_OK;
		...
		char cc[2] = {' ', 0};
        chk_mounted((const TCHAR **)&cc , &fs, 1);

        //fs->fatbase = bsect + nrsv;


        DWORD sectors_to_read = fs->fatbase + fs->fsize * fs->n_fats;
        DWORD end_of_copy_1 = fs->fatbase + fs->fsize;

        for(f_sect = fs->fatbase; f_sect < sectors_to_read; f_sect++) //  Читаем все секторы с FAT
        {
            disk_read(fs->drv, fs->win, f_sect, 1);
               
            if(f_sect < end_of_copy_1)
            {
                calc_crc_1 = CRC16(((BYTE *)fs->win + 2), SS(fs) - 2, calc_crc_1); 
                if(f_sect == fs->fatbase)
                {
                    read_crc_1 = LD_WORD(fs->win);
                }
            }
            else
            {
                calc_crc_2 = CRC16(((BYTE *)fs->win + 2), SS(fs) - 2, calc_crc_2); 
                if(f_sect == end_of_copy_1)
                {
                    read_crc_2 = LD_WORD(fs->win);
                }
            }
        }


        if(read_crc_1 != calc_crc_1 && read_crc_1 != 0XA5A5)
        {
            if(read_crc_2 != calc_crc_2)
			{
				unlock_fs(fs, FR_OK);
				return FR_NO_FILESYSTEM;
			}
            ...
            {
                f_sect = fs->fatbase + fs->fsize;
                //for(f_sect = fs->fatbase; f_sect < sectors_to_read; f_sect++) //  Читаем все секторы с FAT
                //{
                    disk_read(fs->drv, fs->win, f_sect, 1);
                    disk_write(fs->drv, fs->win, (f_sect - fs->fsize), 1);
					unlock_fs(fs, FR_OK);
                    return FR_OK;
               // }
            }
...
)
{
	...
	for (i = 0; i < N_FATS; i++) {
		...
		n = 0xA5A5;//md;								/* Media descriptor byte */
		if (fmt != FS_FAT32) {
			n |= (fmt == FS_FAT12) ? 0x00FF0000 : 0xFFFFFF00;
