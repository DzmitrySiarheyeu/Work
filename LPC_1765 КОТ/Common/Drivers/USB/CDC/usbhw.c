...
/*  uint32_t datt = 0;
  do{
  		datt = RdCmdDat(DAT_SEL_EP(EPNum));
	}while((datt & 0xC0) > 0); */

  LPC_USB->USBCtrl = ((EPNum & 0x0F) << 2) | CTRL_WR_EN;
  //index	= LPC_USB->USBCtrl;

  LPC_USB->USBTxPLen = cnt;
  //index = LPC_USB->USBTxPLen;
...
uint32_t USB_DMA_Setup(uint32_t EPNum, USB_DMA_DESCRIPTOR *pDD) {
  ...
  uint32_t *nxt_tmp;
  nxt_tmp = (uint32_t *)nxt;

  /* Fill in DMA Descriptor */
  *(nxt_tmp++) =  0;                 /* Next DD Pointer */
  *(nxt_tmp++) =  pDD->Cfg.Type.ATLE |
                       (pDD->Cfg.Type.IsoEP << 4) |
                       (pDD->MaxSize <<  5) |
                       (pDD->BufLen  << 16);
  *(nxt_tmp++) =  pDD->BufAdr;
  *(nxt_tmp++) =  pDD->Cfg.Type.LenPos << 8;
  if (iso) {
    *(nxt_tmp) =  pDD->InfoAdr;
  }
