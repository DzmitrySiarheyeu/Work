...
static int WriteUSB(DEVICE_HANDLE handle, const void * dst, int count);
...
static int CreateUSB(void)
{
    LPC_PINCON->PINSEL3 &= ~((3<< 4));   /* P1.18 GoodLink */
	LPC_PINCON->PINSEL3 |=  ((1<< 4));   /* PINSEL3 4.5 = 01 */

	LPC_PINCON->PINSEL4 &= ~((3<<18)        );   /* P2.9 SoftConnect */
	LPC_PINCON->PINSEL4 |=  ((1<<18)        );   /* PINSEL4 18.19 = 01 */
	
	PINSEL_CFG_Type PinCfg;   // Pin configuration for USB Power

    PinCfg.Funcnum = 2;
    PinCfg.OpenDrain = 0;
    PinCfg.Pinmode = 3;			// Pull down
    PinCfg.Portnum = 1;			// P1.30
    PinCfg.Pinnum = 30;
...
static int WriteUSB (DEVICE_HANDLE handle, const void * pSrc, int count)
{
    uint8_t err = 0;
