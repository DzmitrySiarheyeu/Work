...
void RNDIS_Device_ProcessRNDISControlMessage(USB_ClassInfo_RNDIS_Device_t* const RNDISInterfaceInfo)
{
	...
	USB_DEBUG_PRINTF_ONE("RNDISControlMessage - %d\n", le32_to_cpu(MessageHeader->MessageType));
	switch (le32_to_cpu(MessageHeader->MessageType))
	{
		...
		case REMOTE_NDIS_QUERY_MSG:
			...
			USB_DEBUG_PRINTF_ONE("Query_Oid - %d\n", Query_Oid);
                                ...
                                uint16_t* const PacketLength)
{
	...
	*PacketLength = 0;
	...
	err = Endpoint_Read_Stream_LE(&RNDISPacketHeader, sizeof(RNDIS_Packet_Message_t), NULL);
	
	if(err != ENDPOINT_RWSTREAM_NoError)
		return ENDPOINT_RWSTREAM_NoError;
#ifdef DEBUG
	if (le32_to_cpu(RNDISPacketHeader.DataLength) > 1500)
	{
	...
	if(le32_to_cpu(RNDISPacketHeader.DataOffset) > 36)
	{
		Endpoint_StallTransaction();
	...
	*PacketLength = (uint16_t)le32_to_cpu(RNDISPacketHeader.DataLength);
	USB_DEBUG_PRINTF_ONE("RNDISPacketHeader.DataLength %d\n", *PacketLength);
	
	Endpoint_Read_Stream_LE(Buffer, *PacketLength, PacketLength);
	USB_DEBUG_PRINTF_ONE("BytesProcessed %d\n", *PacketLength);
