...
uint8_t iso_buffer[1] ATTR_ALIGNED(4) __DATA(USBRAM_SECTION); // так как не надо то уменьшили до минимума //uint8_t iso_buffer[512] ATTR_ALIGNED(4) __DATA(USBRAM_SECTION);
		...
		if ( EoTIntSt & _BIT(PhyEP) )
		{
			if ( IsOutEndpoint(PhyEP) )                 /* OUT Endpoint */
			{
				...
				//if((usb_data_buffer_OUT_size + usb_data_buffer_OUT_index +
					//			dmaDescriptor[PhyEP].MaxPacketSize) > USB_DATA_BUFFER_TEM_LENGTH)
