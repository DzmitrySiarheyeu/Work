...
void MSC_MemoryRead (void) {
  ...
  DeviceIOCtl(hFlash, SWITCH_TO_RAW, 0);
  DeviceIOCtl(hFlash, SET_FLASH_ADDR, (uint8_t *)&Offset);
  ReadDevice(hFlash, BulkBuf, n);
...
static void mem_write_manage(uint32_t offset, uint32_t len)
{
    uint32_t curr_wr_block = offset / MSC_BlockSize;

    // Если хотим писать в начало блока, его надо предварительно стереть
    if(offset == curr_wr_block * MSC_BlockSize)
    {
        SeekDevice(hFlash, curr_wr_block, 0);
        DeviceIOCtl(hFlash, ERASE_BLOCK, 0);
    }

    DeviceIOCtl(hFlash, SWITCH_TO_RAW, 0);
    DeviceIOCtl(hFlash, SET_FLASH_ADDR, (uint8_t *)&offset);
    WriteDevice(hFlash, BulkBuf,  len);
}
...
void MSC_MemoryWrite (void) {
  ...
  mem_write_manage(Offset, (uint32_t)BulkLen);
...
void MSC_MemoryVerify (void) {
  uint32_t n;
  uint8_t verif_buf[MSC_MAX_PACKET];
  ...
  DeviceIOCtl(hFlash, SWITCH_TO_RAW, 0);
  DeviceIOCtl(hFlash, SET_FLASH_ADDR, (uint8_t *)&Offset);
  ReadDevice(hFlash, verif_buf, BulkLen);

  for (n = 0; n < BulkLen; n++) {
    if (verif_buf[n] != BulkBuf[n]) {
      MemOK = FALSE;
      break;
    }
  }
