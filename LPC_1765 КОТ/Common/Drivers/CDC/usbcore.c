...
uint8_t  EP0Buf[USB_MAX_PACKET0 * 4];
...
__inline uint32_t USB_ReqGetDescriptor (void) {
  ...
  switch (SetupPacket.bmRequestType.BM.Recipient) {
	...
	/*	case USB_DEVICE_QUALIFIER_DESCRIPTOR_TYPE:
          pD = (uint8_t *)USB_QualifierDescriptor;
          for (n = 0; n != SetupPacket.wValue.WB.L; n++) {
            if (((USB_DEVICE_QUALIFIER_DESCRIPTOR *)pD)->bLength != 0) {
              pD += ((USB_DEVICE_QUALIFIER_DESCRIPTOR *)pD)->bLength;
            }
          }
          if (((USB_DEVICE_QUALIFIER_DESCRIPTOR *)pD)->bLength == 0) {
            return (FALSE);
          ...
          EP0Data.pData = pD;
          len = ((USB_DEVICE_QUALIFIER_DESCRIPTOR *)pD)->bLength;
          break; */
	...
	/*	case USB_ENDPOINT_DESCRIPTOR_TYPE:
          pD = (uint8_t *)USB_EPDescriptor;
          for (n = 0; n != SetupPacket.wValue.WB.L; n++) {
            if (((USB_ENDPOINT_DESCRIPTOR *)pD)->bLength != 0) {
              pD += ((USB_ENDPOINT_DESCRIPTOR *)pD)->bLength;
            }
          }
          if (((USB_ENDPOINT_DESCRIPTOR *)pD)->bLength == 0) {
            return (FALSE);
          ...
          EP0Data.pData = pD;
          len = ((USB_ENDPOINT_DESCRIPTOR *)EP0Data.pData)->bLength;
          break; */
        default:
          return (FALSE);
      }
          ...
          switch (pD->bDescriptorType) {
            case USB_CONFIGURATION_DESCRIPTOR_TYPE:
              if (((USB_CONFIGURATION_DESCRIPTOR *)pD)->bConfigurationValue == SetupPacket.wValue.WB.L) {
				...
				USB_ConnectionStatus = USB_Configuration;
                ...
                pTemp = (uint8_t *)pD + ((USB_CONFIGURATION_DESCRIPTOR *)pD)->wTotalLength;
                pD = (USB_COMMON_DESCRIPTOR *)pTemp;
                continue;
              }
              break;
              ...
              if ((SetupPacket.wIndex.WB.L == USB_CDC_CIF_NUM)  ||       /* IF number correct? */
                ...
                switch (SetupPacket.bRequest) {
                  case CDC_SEND_ENCAPSULATED_COMMAND:
	  	 			//	USB_ReadEP(0x00, (uint8_t *)EP0Buf);
					//	EP0Data.pData = EP0Buf;
                    EP0Data.pData = EP0Buf;                              /* data to be received, see USB_EVT_OUT */
                    goto setup_class_ok;
                  case CDC_GET_ENCAPSULATED_RESPONSE:
                    if (CDC_GetEncapsulatedResponse(&SetupPacket, &EP0Data)) {
					//	memcpy(EP0Buf, EP0Data.pData, EP0Data.Count);
                     // EP0Data.pData = EP0Buf;
                      USB_DataInStage();
					 //EP0Data.pData = EP0Buf;                                 /* send requested data */
                      goto setup_class_ok;
                    }
                    break;
...
//                      case AUDIO_REQUEST_SET_RES:
                    ...
                    if ((SetupPacket.wIndex.WB.L == USB_CDC_CIF_NUM)  || /* IF number correct? */
                      ...
                      switch (SetupPacket.bRequest) {
                        case CDC_SEND_ENCAPSULATED_COMMAND:
                          if (CDC_SendEncapsulatedCommand(&SetupPacket, &EP0Data)) {
                            USB_StatusInStage();                         /* send Acknowledge */
						//	CDC_SendEncapsulatedCommand(&SetupPacket, &EP0Data);
                            goto out_class_ok;
                          }
                          break;
                        case CDC_SET_COMM_FEATURE:
