...
extern int CDC_RdInBuf (char *buffer);
extern int CDC_WrInBuf         (const char *buffer, int *length);

extern int CDC_OutBufAvailChar (void);
...
extern uint32_t CDC_SendEncapsulatedCommand  (USB_SETUP_PACKET *setup_packet, USB_EP_DATA *ep_data);
extern uint32_t CDC_GetEncapsulatedResponse (USB_SETUP_PACKET *setup_packet, USB_EP_DATA *ep_data);
