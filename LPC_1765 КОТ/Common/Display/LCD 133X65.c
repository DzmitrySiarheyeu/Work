...
void LCD_init(void)
{
    ...
    LCD_config_send(LCD_JUMP_TO_H_111); //На начальную страницу настроек
    LCD_config_send(LCD_FUNCTION_SET /*| LCD_V*/); //Выйти из power_down_mode (0х04 -> 0), горизонтпльная адресация (0x02 -> 0)
    LCD_config_send(LCD_JUMP_TO_H_110); //На страницу настроек 110
    LCD_config_send(LCD_BIAS_SYSTEM | LCD_BS1); //Смещение 1/9 (размера 133Х65)
    LCD_config_send(LCD_DISPLAY_CONTROL | LCD_D); //Режим отображения - нормальный
    LCD_config_send(LCD_MULTIPLEX_RATE | LCD_M2); //Используем весь ЖКИ
    LCD_config_send(LCD_JUMP_TO_H_111); //На начальную страницу настроек
    LCD_config_send(LCD_JUMP_TO_H_101); //На страницу настроек 101
    LCD_config_send(LCD_HV_GEN_STAGES | LCD_S0); //Внутренний умножитель напряжения - 3*Vdd
    LCD_config_send(LCD_TEMP_COEF | LCD_TC1);//Задаем температурный коэффициент
    LCD_config_send(_LCD_CONTRAST_);//Напряжение Vlcd ~ 7 В
    LCD_config_send(LCD_HV_GEN_CONTROL | LCD_HVE);//Включаем умножитель напряжения
    LCD_config_send(LCD_JUMP_TO_H_111); //На начальную страницу настроек
    LCD_config_send(LCD_JUMP_TO_H_011); //На страницу настроек
    LCD_config_send(LCD_COG_TCP | LCD_TRS | LCD_BRS); //Включаем нормальный порядок заполнения строк
...
void LCD_config_send(uint8_t data)
{
    SeekDevice(hI2C, LCD_WRITE_CONFIG, LCD_ADDR);
    WriteDevice(hI2C, (void *)&data, 1);
}

void LCD_data_send(uint8_t data)
{
    SeekDevice(hI2C, LCD_WRITE_RAM, LCD_ADDR);
    WriteDevice(hI2C, (void *)&data, 1);
}
...
void LCD_set_column(uint8_t Column)
{
     if(Column > 127) 
     {
         LCD_config_send(LCD_JUMP_TO_H_111);
         LCD_config_send(LCD_RAM_PAGE | LCD_SET_RAM_PAGE);
         Column = Column - 128;
     }
     else 
     {
         LCD_config_send(LCD_JUMP_TO_H_111);
         LCD_config_send(LCD_RAM_PAGE);
     }
     
     LCD_config_send(LCD_SET_X_ADDR | Column);
}

void LCD_set_row(uint8_t Row)
{
    LCD_config_send(LCD_JUMP_TO_H_111);
    LCD_config_send(LCD_SET_Y_ADDR | Row);
}
...
{
    uint16_t i = 0;
    for(i = 0;i<1064;i++) LCD_buffer->LCD_Screen_Buff2[i] = 0;
}
...
void LCD_buff_set(uint8_t column, uint8_t row, uint8_t data)
{
    ...
    LCD_buffer->LCD_Screen_Buff1[row][column] = data;
}
...
int16_t deltax, deltay, error, deltaerr, y, x, step;
    ...
    if((X1 > 132) || (Y1 > 64)) return;
    if((X2 > 132) || (Y2 > 64)) return;
    if(X1 > X2) return;
