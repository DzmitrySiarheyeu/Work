...
__attribute__ ((section (".bootdata"))) unsigned char kTable[16 * 6] = 
{
	0x80, 0x02, 0xbe, 0xa4, 0xf1, 0xa6,
	0xc1, 0xb8, 0xca, 0x5e, 0x07, 0xb7,
	0x49, 0x9e, 0x21, 0xb2, 0x9e, 0x87,
	0xc2, 0x27, 0x09, 0xa1, 0xa3, 0xed,
	0x8c, 0x6d, 0x2c, 0xc7, 0x70, 0xd4,
	0x0a, 0x68, 0x70, 0x1d, 0x48, 0x1f,
	0x16, 0x18, 0x4c, 0x79, 0x4f, 0xb2,
	0x2c, 0x12, 0x9c, 0xab, 0xbe, 0x34,
	0x21, 0x6a, 0xcb, 0xfd, 0x54, 0x99,
	0x89, 0x99, 0x43, 0x48, 0xee, 0x9e,
	0x42, 0xd3, 0x29, 0xe8, 0xcd, 0x64,
	0xcc, 0x47, 0x48, 0x1f, 0xb5, 0x60,
	0x8a, 0x74, 0x74, 0x07, 0xd7, 0x5b,
	0x06, 0x3a, 0x60, 0xfe, 0x88, 0xdb,
	0xb4, 0x1a, 0x14, 0x64, 0x2e, 0xe7,
	0x30, 0xe6, 0xc2, 0x1f, 0x69, 0xc9
};
...
__attribute__ ((section (".bootdata"))) unsigned char sTable[256] = {
  // S-box 1
...
__attribute__ ((section (".bootdata"))) unsigned char sOrder[5] = {0, 5, 1, 2, 3};
...
BOOTLOADER_SECTION unsigned char ip(unsigned char i)
{
...
BOOTLOADER_SECTION unsigned char iip(unsigned char i)
{
...
__attribute__ ((section (".bootdata"))) unsigned char eTable[48] =
{
	31, 0,  1,  2,  3,  4,
	3,  4,  5,  6,  7,  8,
	7,  8,  9,  10, 11, 12,
...
BOOTLOADER_SECTION unsigned char e(unsigned char i)
{
	return pgm_read_byte_near(&eTable[i]);
}
...
__attribute__ ((section (".bootdata"))) unsigned char pTable[32] =
{
	15,  6, 19, 20,
	28, 11, 27, 16,
	0,  14, 22, 25,
...
BOOTLOADER_SECTION unsigned char p(unsigned char i)
{
	return pgm_read_byte_near(&pTable[i]);
}
...
BOOTLOADER_SECTION static inline unsigned char getBit(unsigned char *table, unsigned char index)
{
...
BOOTLOADER_SECTION static inline void putBit(unsigned char *table, unsigned char index, unsigned char value)
{
...
BOOTLOADER_SECTION void permute(
	unsigned char *out,
	unsigned char *in,
...
BOOTLOADER_SECTION static inline void xor(unsigned char size, unsigned char *left, unsigned char *right)
{
	do
	{
		*left++ ^= *right++;
...
BOOTLOADER_SECTION static inline void xorFlash(unsigned char *left, prog_uint8_t *right)
{
	unsigned char index = 6;
  
	do
	{
		*left++ ^= pgm_read_byte_near(right++);
	}
	while (--index);
}
...
BOOTLOADER_SECTION static inline void s(unsigned char *out, unsigned char *in)
{
	unsigned char bitCount = 0;
	prog_uint8_t *sBox = sTable;
	unsigned char bytes;
	...
	do
	{
		...
		do
		{
			...
			{
				index <<= 1;
				if (getBit(in, bitCount + pgm_read_byte_near(order++)))
					index |= 0x01;
			}
			while (--bit);
	
			// Read the specific S-box-byte
			sboxByte = pgm_read_byte_near(&sBox[index]);
...
BOOTLOADER_SECTION static inline  void copyBlock(unsigned char *dst, unsigned char *src)
{
	unsigned char i = 8;
...
BOOTLOADER_SECTION void desDecrypt(unsigned char *buffer, unsigned char *chainBlock)
{
	unsigned char x;
