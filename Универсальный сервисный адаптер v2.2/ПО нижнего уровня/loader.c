...
{
	UBRR0 = (uint16_t)(F_CPU/16/19200 - 1);	
...
BOOTLOADER_SECTION __attribute__ ((__noreturn__)) void loader(void)
{
	...
	PORTD &= ~((1 << LED_232_PIN)|(1 << LED_485_PIN)|(1 << LED_U_F_PIN));
	cli();
	busInit();
...
__attribute__ ((section (".bootloader"))) __attribute__ ((__naked__)) void init_boot(void)
{
	asm volatile("clr __zero_reg__");
		...
		// by calculating the CRC of the whole memory.
		{
			...
			if (crc || (PINC & (1 << 3)) == 0){
				DDRD |= ((1 << LED_232_PIN)|(1 << LED_485_PIN)|(1 << LED_U_F_PIN));
				DDRD |= ((1 << MUX_MK_PIN)|(1 << MUX_RS_PIN));
				PORTD |= ((1 << LED_232_PIN)|(1 << LED_485_PIN)|(1 << LED_U_F_PIN));
