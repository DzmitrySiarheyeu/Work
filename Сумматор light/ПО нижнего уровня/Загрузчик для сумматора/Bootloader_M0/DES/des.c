...
const unsigned char sOrder[5] = {0, 5, 1, 2, 3};
...
void xorFlash(unsigned char *left, unsigned char *right)
{
...
void s(unsigned char *out, unsigned char *in)
{
	...
	unsigned char *sBox = (unsigned char *)sTable;
	unsigned char bytes;
...
void desDecrypt(unsigned char *buffer, unsigned char *chainBlock)
{
	...
	{    
		...
		xorFlash(temp1, (unsigned char *)&kTable[6 * (x & DES_MASK)]);
		s(temp2, temp1);
